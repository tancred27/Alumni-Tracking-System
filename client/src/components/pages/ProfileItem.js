...
const ProfileItem = (props) => {
        ...
        <form className="form-container" onSubmit={onSubmit}>
          <div className="form-group">
            ...
            <input
              id="1"
              type="text"
              placeholder={alumniContext.user.info}
              name="name"
              onChange={onChange}
            />
          </div>
          <div className="form-group">
            ...
            <input
              id="2"
              name="company"
              placeholder={alumniContext.user.company}
              onChange={onChange}
              type="text"
            />
          </div>
  ...
  const renderButton = () => {
    ...
    if (friends.indexOf(_id) != -1) {
      return (
        <center>
          <button className="btn btn-outline-success disabled">Friends</button>
        </center>
      );
  ...
  return (
    <div className="card grid-2">
      <div className="all-center">
        <img
          src="https://i.4pcdn.org/tv/1485796767156.jpg"
          className="round-img"
          alt=""
          style={{ width: "150px" }}
        />
        <h1>{name}</h1>
        {props.alumni && props.notf ? <div>{renderButton()}</div> : <div></div>}
        {!user.isUser ? (
          <div>
            <center>
              <Link to="/">
