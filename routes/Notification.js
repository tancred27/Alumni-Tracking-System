...
router.put("/req", auth, async (req, res) => {
  ...
  try {
    ...
    if (!notf1 || !notf2)
      return res.status(404).json({ msg: "REQUEST_ACCEPT_ERROR" });
    try {
      if (
        notf2.Request.indexOf(`${request}`) != -1 &&
        notf1.Accept.indexOf(`${accept}`) != -1
      ) {
        field1 = {};
        field2 = {};
    ...
    /*if(notf1.id!==req.user.id) 
       ...
       */
    notf1 = await Notification.findByIdAndUpdate(
      notf1._id,
      { $set: field1 },
      { new: true }
    );
    notf2 = await Notification.findByIdAndUpdate(
      notf2._id,
      { $set: field2 },
      { new: true }
    );
    res.send({
      friends: notf1.Friends,
      accept: notf1.Accept,
      request: notf1.Request,
    });
  } catch (err) {
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    res.send({
      friends: notf.Friends,
      accept: notf.Accept,
      request: notf.Request,
    });
  } catch (err) {
    console.error(err.name + ":" + err.message);
    res.status(500).json({ msg: "SERVER_ERROR" });
